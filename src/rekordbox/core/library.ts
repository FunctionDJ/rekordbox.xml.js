...
export class Library implements XMLSerializable<RekordboxXML> {
	...
	public addTrack(track: Track): void {
		if (this.hasTrack(track)) {
			// todo maybe check for same location too?
			throw new IDCollisionError();
		}
	...
	async save(
		filePath: string
	): Promise<{ buildTime: string; writeTime: string }> {
		...
		const [buildTime, xmlString] = measureSync(() =>
			builder.build(this.serialize())
		);
		const patchedString = xmlString.replaceAll(/&(?!\w+;)/gm, "&amp;");
		const [writeTime] = await measure(
			async () => await fs.writeFile(filePath, patchedString)
		);
