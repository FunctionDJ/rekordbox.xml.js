...
export class Playlist extends IdentifiableNode implements XMLSerializable<IPlaylist> {
	constructor (
	...
	static createFromXML (
		...
		const tracks = (data.TRACK ?? []).map(t => {
			...
			if (track === null) { // todo maybe add to library if missing?
				throw new Error(`Could not find track "${t.Key}" in track pool while populating playlist.`);
			}
	...
	removeTrack (track: Track, all = false): void {
		if (!this.hasTrack(track)) {
			throw new Error(`Can't remove track: Playlist "${this.name}" doesn't contain track "${track.toString()}"`);
		}

		if (this.getCountOfTrack(track) > 1 && !all) {
			throw new Error(`Can't remove track because "all = true" was not passed: Playlist "${this.name}" contains more than 1 instance of track "${track.toString()}"`);
		}
	...
	addTrack (track: Track, ignoreDuplicate = false): void {
		if (!ignoreDuplicate && this.hasTrack(track)) {
			throw new Error(`Could not add track: Playlist "${this.name}" already contains track "${track.toString()}"`);
		}
