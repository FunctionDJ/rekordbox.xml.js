...
export const loadInputXml = async (inputXmlFile: string): Promise<Library> => {
	try {
		...
		console.error("An error occurred while loading the input .xml file and creating the library:", error);
		process.exit(1);
	}
};

export const runScript = async (script: RXScript, library: Library): Promise<undefined|Library> => {
	try {
		return await script.run(library);
	} catch (error) {
...
export const saveOutputXml = async (library: Library, outputXmlFile: string): Promise<void> => {
	try {
		await library.save(outputXmlFile); // TODO differentiate between XML generation and FS call
	} catch (error) {
		console.error("An error occurred while saving the output .xml file:", error);
		process.exit(1);
	}
};
